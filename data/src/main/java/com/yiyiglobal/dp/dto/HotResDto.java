...
public class HotResDto implements Serializable, Comparable<HotResDto>  {
    private Integer id;
    ...
    public HotResDto(String resName, Integer categoryId) {
    ...
    @Override
    public int compareTo(HotResDto s) {
        //return -1; //-1表示放在红黑树的左边,即逆序输出
        //return 1;  //1表示放在红黑树的右边，即顺序输出
        //return o;  //表示元素相同，仅存放第一个元素
        if(s.id.equals(this.id)){// id 一样排重
            return  0;
        }
        // id 不一样，排序
        // 主要条件 24小时内评论数
        int num=s.dayReviewNum-this.dayReviewNum;
        // 总评论数
        int num1=num==0?s.reviewNum-this.reviewNum:num;
        // 评论数相等的话比较资源名称
        int num2=num1==0?s.resName.compareTo(this.resName):num;
        return num2;
    }
}